...
export default class BitbucketIssueSelector extends PureComponent {
    static propTypes = {
    ...
    searchIssues = (text) => {
        ...
        return Client.searchIssues(textEncoded).then((data) => {
            ...
            return data.map((item) => {
                const repoParts = item.repository.links.html.href.split('/');
                let prefix = '';
                if (repoParts.length >= 2) {
                    prefix = repoParts[repoParts.length - 2] + '/' + repoParts[repoParts.length - 1] + ', ';
                }
                return ({value: item, label: prefix + '#' + item.id + ': ' + item.title, isDisabled: item.locked});
            });
        }).catch((e) => {
